...
from conversation import ChatLine, Conversation
            ...
            if u_type == "chatLine":
                ...
                if not board.is_game_over() and is_engine_move(game, moves):
                    if config.get("fake_think_time") and len(moves) > 9:
                        delay = (
                            min(game.clock_initial, game.my_remaining_seconds()) * 0.015
                        )
                    ...
                    if best_move == None:
                        best_move = engine.search(
                            board, upd["wtime"], upd["btime"], upd["winc"], upd["binc"]
                        )
    ...
    with chess.polyglot.open_reader(book) as reader:
        try:
            ...
            if selection == "weighted_random":
                ...
                move = reader.choice(
                    board, minimum_weight=config.get("min_weight", 1)
                ).move()
